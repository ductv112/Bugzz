...
requirements:
  covered:
    ...
    - id: CAM-08
      status: pass
      note: "BboxIouTracker-assigned integer ID stable across 60+ consecutive frames (verified on Xiaomi 13T / HyperOS 2026-04-20 via FaceTracker logcat during Phase 3 HANDOFF Step 9, 1120+ consecutive frames, sole unique ID = {id=0}, zero id=null entries). ML Kit native trackingId remains null under CONTOUR_MODE_ALL per ADR-01 — this is the documented baseline, not a bug. Re-verification closed ADR-01 follow-up #4. See 03-05-SUMMARY.md Task 3 device verification."
    - id: CAM-09
      status: pass
...
| 5 | Face `trackingId` remains stable for the same face across 60+ consecutive frames; 1€ filter smooths landmark jitter to <1px/frame on a still head | ✅ Complete | **Closed via GAP-02-A / 02-gaps-01 (Phase 2 relaxed acceptance) + re-verified at Phase 3 exit (full closure).** Phase 2: `.enableTracking()` removed (commit `98e032a`) — documented Google ML Kit limitation under `CONTOUR_MODE_ALL`; relaxed acceptance: boundingBox centroid continuity confirmed on device. Phase 3 full closure: `BboxIouTracker`-assigned integer ID `id=0` stable across 1120+ consecutive frames on Xiaomi 13T / HyperOS (Phase 3 HANDOFF Step 9, 2026-04-20); zero `id=null` entries. 1€ filter unit tests (4/4 `OneEuroFilterTest`) GREEN. ADR-01 all 4 follow-ups closed. |
...
| CAM-08 | Face identity remains stable across frames for the same face | ✅ Complete | **BboxIouTracker-assigned integer ID stable across 60+ consecutive frames (verified on Xiaomi 13T / HyperOS via `FaceTracker` logcat during Phase 3 HANDOFF Step 9, 2026-04-20). ML Kit native `trackingId` remains null under `CONTOUR_MODE_ALL` per ADR-01 — the null is documented baseline, not a bug. Re-verification: `id=0` sole unique ID across 1120+ consecutive frames; zero `id=null` entries. See `.planning/phases/03-first-filter-end-to-end-photo-capture/03-05-SUMMARY.md` Task 3 device verification. ADR-01 follow-up #4 CLOSED.** (Previously: relaxed acceptance per [02-ADR-01](./02-ADR-01-no-ml-kit-tracking-with-contour.md) — boundingBox centroid continuity for Phase 2; `-1` sentinel replaced by BboxIouTracker in Phase 3.) |
